    ...
    def s_armony(self):
        ...
        for _ in range(10000):
            for columna in range(self.dim):
                ...
                if ran1 < self.hmcr:
                    indiceMA = np.random.randint(0,self.arm_rang)
                    if ran2 < self.par:
                        memoriaArm2 = self.matriz[indiceMA,columna] + self.bw * (2*np.random.random()-1)
                        matriz_extrac =  self.matriz[indiceMA]
                        matriz_extrac[columna] = memoriaArm2
                        self.matriz = self.verificar(matriz_extrac)
                    else:
                        matriz_extrac =  self.matriz[indiceMA]
                        self.matriz = self.verificar(matriz_extrac)
                else:
